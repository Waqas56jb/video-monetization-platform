...
export default function Watch() {
  ...
  const p = playbackRouteMatches(playbackRow, v, videoId) ? playbackRow : null
  ...
  /**
   * The player's shape, known before the video row arrives.
   *
   * The playback payload carries width and height now, so the frame is built at
   * the right aspect on the first render and does not change when `/api/videos`
   * lands. The previous attempt at mounting early skipped this: the wrapper
   * rendered 16:9, then swapped its class and its --player-aspect /
   * --player-ratio underneath a cross-origin iframe mid-bootstrap, and
   * Cloudflare's player paid about 900 ms for the resize — more than the wait
   * that mounting early had removed.
   *
   * The video row still wins when it is present, so nothing changes for a page
   * that already has it; playback is the earlier source of the same numbers.
   */
  const shape = videoShape(
    v?.width || p?.width || measured?.width,
    v?.height || p?.height || measured?.height
  )
